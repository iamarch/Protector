...
import pyrogram
from Protectorfilters import AndFilter, Filter, InvertFilter, OrFilter

from Protector.action import BotAction
...
def check_filters(_filters: Union[Filter, CustomFilter], protector: "Protector") -> None:
    """ Recursively check filters to set :obj:`~Protector` into :obj:`~CustomFilter` if needed """
    if isinstance(_filters, (AndFilter, OrFilter, InvertFilter)):
        check_filters(_filters.base, protector)
    if isinstance(_filters, (AndFilter, OrFilter)):
        check_filters(_filters.other, protector)

    # Only accepts CustomFilter instance
    if getattr(_filters, "include_bot", False) and isinstance(_filters, CustomFilter):
        _filters.protector = Protector
