...
class TelegramBot(Base):
    ...
    async def init_client(self: "Protector") -> None:
    ...
    async def start(self: "Protector") -> None:
        """Start client"""
        ...
        await self.connect_db("Protector")
        self._load_language()
        try:
            subplugins = [
                importlib.import_module("Protector.plugins." + info.name, __name__)
                for info in pkgutil.iter_modules(["Protector/plugins"])
            ]
